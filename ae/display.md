...
```ae
Circle().size(40).fill("#FF4D4F")
```

```swift
Circle().frame(width: 40, height: 40).fill(Color(hex: "#FF4D4F"))
```
...
```ae
RoundedRectangle().w(200).h(60).fill("#1E2333").radius(12)
```
...
> RichText、RichEditor、CodeBlock、Markdown 组件已移至 [code-markdown.md](code-markdown.md)。
...
> Canvas 组件及其 DrawItem/DisplayList/Block 绘制系统已移至 [canvas.md](canvas.md)。相关 Rust API 详见 [Canvas 绘制与 Block 布局](../logic/canvas-drawing.md)。

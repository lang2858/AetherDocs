...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| height | num | 否 | 42 | 标题栏高度 (pt) |
| bg | str | 否 | "#1E1E1E" | 标题栏背景色 |

支持子组件。内容完全自由：用户用 HStack/Spacer/Text/Icon 等组件自行布局。
...
Toolbar 生成 `HStack(spacing: 0)` 外壳及修饰符，与 content 区域一起包裹在 `VStack(spacing: 0)` 中：
...
使用自定义 Toolbar 时，App 层自动：

- 隐藏系统标题栏（`titlebarAppearsTransparent` + `titleVisibility: .hidden` + `fullSizeContentView`）
- 红绿灯按钮垂直居中到自定义标题栏高度
- 使用 ZStack 替代 TabView 切换 tab（避免系统 tab bar 冲突）
- 双击标题栏最大化窗口
